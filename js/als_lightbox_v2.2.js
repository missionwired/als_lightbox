...
// Searches for selector ('script[data-id="als_fancybox2_js"], #als_fancybox2_js') or similar.
...
// 2 - alsLightbox.config.paths: Paths to dependencies- jQuery, fancybox2, and ALS lightbox CSS assets
...
alsLightbox.config.paths = {
	...
	"fancybox2_js": "https://s3.amazonaws.com/annelewisllc/lightbox/src/bower_components/fancybox/source/jquery.fancybox2.als.js",
	"fancybox2_css": "https://s3.amazonaws.com/annelewisllc/lightbox/src/bower_components/fancybox/source/jquery.fancybox2.css",
...
// 	"fancybox2_js": "bower_components/fancybox2/source/jquery.fancybox2.pack.als.js",
// 	"fancybox2_css": "bower_components/fancybox2/source/jquery.fancybox2.css",
...
alsLightbox.launch = function () {
  ...
  // Technique from http://css-tricks.com/snippets/jquery/load-jquery-only-if-not-present/
	...
	if ( ( typeof jQuery === 'undefined' ) || ( jQuery.fn.jquery < enforceMinJQueryVersion ) ) {
        ...
        (function($) { load_fancybox2($); })(alsLightbox.jQuery);
	...
	// Load fancybox2 if not present.

	function load_fancybox2($) {

		if(typeof $.fancybox2 === 'function') {
			 load_fancybox2_css($);
			 execute_fancybox2($);
		} else {
      // Use $.getScript to make sure that the current local version of $ (alsLightbox.jQuery) stays in scope.
      $.getScript(alsLightbox.config.paths.fancybox2_js, function(){
        load_fancybox2_css($);
        execute_fancybox2($);
      });
		}
	...
	// Load fancybox2 CSS if not present.

	function load_fancybox2_css($) {
		if (!$('link[href$="jquery.fancybox2.css"]').length) {
			var fancybox2_base_style_tag = document.createElement('link');
				fancybox2_base_style_tag.rel  = 'stylesheet';
				fancybox2_base_style_tag.type = 'text/css';
				fancybox2_base_style_tag.media = 'screen';
				fancybox2_base_style_tag.href = alsLightbox.config.paths.fancybox2_css;
				fancybox2_base_style_tag.id = 'als_fancybox2_base_style_tag';
			document.getElementsByTagName('head')[0].appendChild(fancybox2_base_style_tag);
		}
	}

	// Execute fancybox2 implementation.

	function execute_fancybox2($) {
	...
	Disable ability for child iframe to resize itself.
		...
		eventer(messageEvent,function(e) {
			if (/fancybox2\.height\('{0,1}\d+'{0,1}\)/.test(e.data)) {
				$(document).ready(function() {

					postMessage_height = parseInt(e.data.match(/fancybox2\.height\('{0,1}(\d+)'{0,1}\)/)[1]);
					fancybox2_change_height(postMessage_height);

				});
			}
		},false);

		function fancybox2_change_height(h) {
			$(".als-fancybox2-inner").css({
				height: h
			});
		}
		...
		eventer(messageEvent,function(e) {
			if (/fancybox2\.width\('{0,1}\d+'{0,1}\)/.test(e.data)) {
				$(document).ready(function() {

					postMessage_width = parseInt(e.data.match(/fancybox2\.width\('{0,1}(\d+)'{0,1}\)/)[1]);
					fancybox2_change_width(postMessage_width);

				});
			}
		},false);

		function fancybox2_change_width(w) {
			$(".als-fancybox2-inner, .als-fancybox2-wrap").css({
				width: w
			});
		}
	...
	------- */
		...
		$(document).ready(function( $ ) {
			...
			$("#als_lightbox").fancybox2({
				...
				tpl: {
					wrap     : '<div class="als-fancybox2-wrap fancybox2-wrap" tabIndex="-1"><div class="als-fancybox2-skin fancybox2-skin"><div class="fancybox2-outer"><div class="fancybox2-inner als-fancybox2-inner"></div></div></div></div>',
				}
	// 			, onUpdate: function() {
	//					// Necessary to allow child frame to resize itself.
	//  				if(postMessage_height != 0) { fancybox2_change_height(postMessage_height); }
	//  				if(postMessage_width != 0) { fancybox2_change_width(postMessage_width); }
	// 			}
			});
